...
export const applyAccent = (colors: RGB[]): void => {
	...
	s.setProperty('--accent-rgb', colors[1]?.join(', ') ?? null);
	s.setProperty('--accent-2-rgb', colors[2]?.join(', ') ?? null);
	s.setProperty('--accent-3-rgb', colors[0]?.join(', ') ?? null);
	s.setProperty('--accent-4-rgb', colors[3]?.join(', ') ?? null);
};
