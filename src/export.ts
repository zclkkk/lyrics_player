...
export const createBlobUrlFromRemote = async (
  url: string,
  mimeType: string,
) => {
...
export const releaseDisplayStream = (state: AppState) => {
  if (state.displayStream) {
    for (const track of state.displayStream.getTracks()) {
      track.stop();
    }
...
export const cleanupFfmpegFiles = async (
  ffmpeg: FFmpeg,
  fileNames: string[],
) => {
  await Promise.all(
...
export const observeMediaRecorderStart = (mediaRecorder: MediaRecorder) => {
  ...
  const handleError = () =>
    finalize(() =>
      rejectStart(
        mediaRecorder.error || new Error("MEDIA_RECORDER_START_FAILED"),
      ),
    );
